# -*- coding: utf-8 -*-
"""
Created on Sun Feb  3 15:35:05 2019
...
DelT = data[:,5];
x = DelT;
Qh = data[:,12]; #Q_Ip means (Qc-remove)/Ip
y = Qh;
...
# Labels
plt.ylabel("Heat power $Q_{h-power}$ (watts)")
plt.xlabel('${\Delta}T$ (kelvin)')
plt.legend(handles=[data_plot,fit_plot],loc='best',fontsize=10)
#plt.title('thermalConductance from $Q_{h-power}$ vs. ${\Delta}T$')

# Save figure(fig) = graph
#plt.savefig(name,format,dpi,bbox_inches)
#plt.savefig('thermalConductance_weightedLinFitNchiSqrd_trial2.png', format='png', dpi=300, bbox_inches='tight')
plt.savefig('thermalConductance_weightedLinFitNchiSqrd_trial2_FINAL.png', format='png', dpi=300, bbox_inches='tight')
